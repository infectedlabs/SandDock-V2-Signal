#!/usr/bin/env node
/**
 * BTC BACKFILL - Proper offset pagination (1000 rows per page)
 */
...
async function getAllBTCSignals() {
  console.log('⚡ Loading ALL BTCUSDT signals (paginated)...');
  let allSignals = [];
  let page = 0;
  const pageSize = 1000;
  ...
  while (true) {
    ...
    if (!signals || signals.length === 0) {
      console.log(`\n✓ ${allSignals.length} BTCUSDT signals loaded\n`);
      break;
    }

    allSignals.push(...signals);
    page++;
    process.stdout.write(`${allSignals.length}..`);
  }
...
function processAllCloses(allSignals) {
  console.log('\n⚡ Closing all BTC signals at TP (+1.5%)...');
...
async function main() {
  ...
  console.log('🚀 BTC BACKFILL (PAGINATED)\n' + '='.repeat(70) + '\n');
